...
def calculate_energy(state_dict, all_rows, all_cols):
    ...
    for (r, c), item in state_dict.items():
        groups[(item['type'], item['name'], item['treatment'])].append((r, c))
        
    for (itype, name, trt), coords in groups.items():
...
# 🖥️ Streamlit 界面逻辑
# ==========================================
st.title("🧪 TDIMP 核心阵列系统 | 静态看板与质量诊断")
...
tab1, tab2 = st.tabs(["🧬 生物学因子设计 & 排布", "🖱️ 可用空间精细控制 (Mask编辑器)"])
...
with tab1:
    if "df_genes" not in st.session_state:
        st.session_state.df_genes = pd.DataFrame({
            "靶点名称 (Target)": ["Gene_1", "Gene_1", "Gene_2", "Gene_2"],
            "处理条件 (Treatment)": ["GCG", "AMPK", "GCG", "AMPK"],
            "复孔数量": [5, 5, 5, 5]
        })
    if "df_ctrls" not in st.session_state:
        st.session_state.df_ctrls = pd.DataFrame({
            "对照名称 (Control)": ["NTC", "Vehicle"],
            "处理条件 (Treatment)": ["None", "DMSO"],
            "复孔数量": [6, 4]
        })

    col_L, col_R = st.columns(2)
    with col_L:
        st.subheader("🎯 实验靶点组 (Gene x Treatment)")
        edited_genes = st.data_editor(st.session_state.df_genes, num_rows="dynamic", use_container_width=True)
    with col_R:
        st.subheader("⚖️ 阴阳性对照组 (Controls)")
    ...
    if run_algorithm:
        ...
        for _, row in edited_genes.iterrows():
            if pd.notna(row["靶点名称 (Target)"]):
                items_to_place.extend([{'type': 'gene', 'name': row["靶点名称 (Target)"], 'treatment': str(row.get("处理条件 (Treatment)", "None"))}] * int(row["复孔数量"]))
        for _, row in edited_ctrls.iterrows():
            if pd.notna(row["对照名称 (Control)"]):
                items_to_place.extend([{'type': 'ctrl', 'name': row["对照名称 (Control)"], 'treatment': str(row.get("处理条件 (Treatment)", "None"))}] * int(row["复孔数量"]))

        if len(items_to_place) > total_active_wells:
            st.error(f"❌ 空间溢出！分配 {len(items_to_place)} 孔，但可用仅 {total_active_wells} 孔。")
        else:
            with st.spinner("🔥 模拟退火引擎运作中，正在逼近物理能量极小值..."):
            ...
            for (r, c), item in best_state.items():
                label = f"{item['name']}_{item['treatment']}" if item['treatment'] != "None" else item['name']
                if item['type'] == 'gene': gene_coords[label].append((r, c))
                else: ctrl_coords[label].append((r, c))
            ...
            for r in all_rows:
                ...
                for c in all_cols:
                    if (r, c) not in available_wells:
                        ...
                        if item:
                            val = f"{item['name']}_{item['treatment']}" if item['treatment'] != "None" else item['name']
                            if (r, c) in conflicts:
                                val += " ⚠️"
                            row_data.append(val)
                        else:
            ...
            st.success(f"✅ 排布计算完成！耗时 {elapsed:.3f} 秒，迭代 {iters} 次。")
            ...
            st.markdown("<div style='margin-top: 30px; text-align: right; color: #94a3b8; font-size: 0.8em; font-family: monospace;'>Powered by TDIMP Optimization Engine | Designed by Fxq</div>", unsafe_allow_html=True)
